...
public class  GlobalExceptionHandler {
    ...
    @ExceptionHandler(PlanBuildException.class)
    public ResponseEntity<ApiErrorResponse> handlePlanBuild(PlanBuildException ex, HttpServletRequest req) {
        return build(HttpStatus.BAD_REQUEST, ex.getMessage(), req, null);
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleMethodArgNotValid(MethodArgumentNotValidException ex,
        ...
        List<ApiErrorResponse.FieldErrorItem> items = ex.getBindingResult()
                ...
                .map(fe -> new ApiErrorResponse.FieldErrorItem(
                        fe.getField(),
                        fe.getDefaultMessage() != null ? fe.getDefaultMessage() : "Ошибка валидации"
                ))
                .toList();

        String msg = "Некорректные данные: " + items.size() + " ошибка(и)";
    ...
    public ResponseEntity<ApiErrorResponse> handleConstraintViolation(ConstraintViolationException ex,
        ...
        String msg = "Нарушение ограничений: " + items.size() + " ошибка(и)";
        return build(HttpStatus.BAD_REQUEST, msg, req, items);
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleUnreadable(HttpMessageNotReadableException ex, HttpServletRequest req) {
        return build(HttpStatus.BAD_REQUEST, "Некорректный JSON в запросе", req, null);
    }

    @ExceptionHandler(MissingServletRequestParameterException.class)
    public ResponseEntity<ApiErrorResponse> handleMissingParam(MissingServletRequestParameterException ex,
                                                               HttpServletRequest req) {
        return build(HttpStatus.BAD_REQUEST, "Пропущен обязательный параметр: " + ex.getParameterName(), req, null);
    }

    @ExceptionHandler(HttpRequestMethodNotSupportedException.class)
    public ResponseEntity<ApiErrorResponse> handleMethodNotSupported(HttpRequestMethodNotSupportedException ex,
                                                                     HttpServletRequest req) {
        return build(HttpStatus.METHOD_NOT_ALLOWED, "Метод не поддерживается: " + ex.getMethod(), req, null);
    }

    @ExceptionHandler(HttpMediaTypeNotSupportedException.class)
    public ResponseEntity<ApiErrorResponse> handleMediaType(HttpMediaTypeNotSupportedException ex,
                                                            HttpServletRequest req) {
        return build(HttpStatus.UNSUPPORTED_MEDIA_TYPE, "Неподдерживаемый тип данных запроса", req, null);
    }
