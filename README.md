...
- `docs/practice_report/Отчет_о_преддипломной_практике_Демин.docx` - оформленный отчет по преддипломной практике по проекту Adaprive Learning Navigator.
- `docs/practice_report/Доклад_и_структура_презентации.md` - текст доклада на 5-7 минут и структура презентации из 12 слайдов.
- `docs/practice_report/generate_practice_report.py` - генератор Word-отчета, фиксирующий структуру, титульные реквизиты, таблицы и приложения.

## Аннотация

Adaprive Learning Navigator — это учебный веб-сервис для построения персональных маршрутов изучения IT-направлений. Пользователь выбирает готовую roadmap-карту из каталога, отмечает уже знакомые темы, задаёт количество часов в неделю и получает недельный план обучения. Внутри выбранной темы доступны описание, ресурсы и ИИ-наставник в формате чата.
...
10. На странице roadmap пользователь может открыть тему, посмотреть ресурсы и задать вопрос ИИ-наставнику.
...
- вкладка обзора темы;
- вкладка ресурсов;
- ИИ-наставник по выбранной теме;
- быстрые промпты ИИ-наставника;
- режим проверки знаний через ИИ-наставника;
...
- `ai` — DTO, prompt builder, генерация AI-route и ИИ-наставник;
...
- drawer-панель деталей темы;
- вкладки `Обзор`, `Ресурсы`, `ИИ-наставник`;
...
| Метод | Endpoint | Назначение |
|---|---|---|
| `POST` | `/api/roadmaps/{roleId}/topics/{topicId}/tutor` | Задать вопрос ИИ-наставнику по выбранной теме |
...
ИИ-наставник получает контекст конкретной roadmap и темы. Prompt строится так, чтобы AI отвечал по выбранной теме, учитывал prerequisites, ресурсы и предыдущие реплики текущего диалога.
...
В текущем UI отдельная вкладка quiz убрана. Проверка знаний для пользователя перенесена в ИИ-наставника. При этом backend Quiz API остаётся реализованным: он создаёт базовый quiz для темы при первом запросе, хранит вопросы, варианты и попытки.
...
- `AiTutorController` — ИИ-наставник по теме;
...
ИИ-наставник реализован как чат внутри drawer-панели выбранной темы.
...
ИИ-наставник не строит roadmap и не меняет БД. Он отвечает только в контексте выбранной темы.
...
В текущем frontend отдельная вкладка `Квиз` не используется. Пользовательская проверка знаний вынесена в ИИ-наставника.
...
### Почему ИИ-наставник не сохраняет историю в БД
...
Пользовательский UI проверки знаний перенесён в ИИ-наставника, но backend Quiz API реализован и протестирован. Он оставлен как рабочий слой, потому что связан с таблицами quiz и попытками пользователя.
...
При изменении функционала, API, схемы БД, frontend-структуры, ИИ-наставника, планировщика или roadmap-логики README нужно обновлять вместе с кодом.
